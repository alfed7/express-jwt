...
module.exports = function(options) {
  ...
  var logger = options.logger || nilLogger;

  if (!isFunction(secretCallback)){
    logger('wrapStaticSecretInCallback');
    secretCallback = wrapStaticSecretInCallback(secretCallback);
  }

  logger('secretCallback body', secretCallback);
  ...
  var middleware = function(req, res, next) {
    var token;
    var loggerInt = options.logger || nilLogger;

    loggerInt('middleware');
    ...
    loggerInt('options.getToken', options.getToken);
    if (options.getToken && typeof options.getToken === 'function') {
      try {
        token = options.getToken(req);
        loggerInt('token', token);
      } catch (e) {
        return next(e);
      }
